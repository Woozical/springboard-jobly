...
describe("POST /users", function () {
  ...
  test("forbidden for users: create admin", async function() {
    const resp = await request(app)
        .post("/users")
        .send({
          username: "u-new",
  ...
  test("forbidden for users: create non-admin", async function() {
    const resp = await request(app)
        .post("/users")
        .send({
          username: "u-new",
...
describe("GET /users/:username", function () {
  ...
  test("works for if :username == logged in user", async function () {
    const resp = await request(app)
      .get(`/users/u2`)
      .set("authorization", `Bearer ${userToken}`);
    expect(resp.body).toEqual({
  ...
  test("forbidden if :username != logged in user", async function () {
    const resp = await request(app)
      .get(`/users/u1`)
      .set("authorization", `Bearer ${userToken}`);
    expect(resp.statusCode).toEqual(403);
...
describe("PATCH /users/:username", () => {
  ...
  test("works if :username == logged in user", async function () {
    const resp = await request(app)
        .patch(`/users/u2`)
        .send({
          firstName: "New",
